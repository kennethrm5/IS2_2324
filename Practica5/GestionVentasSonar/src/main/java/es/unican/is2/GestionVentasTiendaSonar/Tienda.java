...
public class Tienda {
	...
	private static final String TOTAL_VENTAS_MES_LABEL = " TotalVentasMes: ";
	private static final String TOTAL_COMISION_LABEL = " TotalComision: ";
	...
	private void vuelcaDatos() throws DataAccessException {
	    ...
	    try {
	        ...
	        for (Vendedor v : senior) {
	            ...
	            out.println(
	                    "  " + NOMBRE_LABEL + v1.getNombre() + ID_LABEL + v1.getId() + DNI_LABEL + v1.dni()
	                            + TOTAL_VENTAS_MES_LABEL + v1.getTotalVentas() + TOTAL_COMISION_LABEL + v1.getComision());
	        }
	        out.println();
	        out.println("Junior");
	        for (Vendedor v : junior) {
	            ...
	            out.println(
	                    "  " + NOMBRE_LABEL + v2.getNombre() + ID_LABEL + v2.getId() + DNI_LABEL + v2.dni()
	                            + TOTAL_VENTAS_MES_LABEL + v2.getTotalVentas() + TOTAL_COMISION_LABEL + v2.getComision());
	        }
	        out.println();
	        ...
	        for (Vendedor v : practicas) {
	            ...
	            out.println(
	                    "  " + NOMBRE_LABEL + v3.getNombre() + ID_LABEL + v3.getId() + DNI_LABEL + v3.getDni()
	                            + TOTAL_VENTAS_MES_LABEL + v3.getTotalVentas());
	        }
	    } catch (IOException e) {
	        throw new DataAccessException();
	    } finally {
